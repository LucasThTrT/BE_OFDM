...
mat_OFDM = [symboles_OFDM(end-3:end,:); symboles_OFDM];  % Ajout en tête des porteuses !!
...
% Implantation de l'égaliseur de type ZFE
% Génération de l'égaliseur H_ZFE via la FFT de h limité a N
C = fft(h, N);
H_ZFE = 1./C;
Mat_recu_ZFE = mat_recu_freq .* H_ZFE.';

signal_traitement = reshape(Mat_recu_ZFE, 1, n);
...
title('Constellation ZFE de la 5ème porteuse');
...
title('Constellation ZFE de la 12ème porteuse');
xlabel('Partie réelle');
ylabel('Partie imaginaire');
axis equal;
grid on;
...
disp("TEB ZFE théorique = " + TEB);




% Implantation de l'égaliseur de type ML
% Génération de l'égaliseur H_ML via la FFT de h limité a N
H_ML = C';
Mat_recu_ML = H_ML .* mat_recu_freq;

signal_traitement = reshape(Mat_recu_ML, 1, n);
...
subplot(2, 1, 1);
plot(real(constellation_porteuse_5), imag(constellation_porteuse_5), 'x');
title('Constellation ML de la 5ème porteuse');
xlabel('Partie réelle');
ylabel('Partie imaginaire');
axis equal;
grid on;
...
subplot(2, 1, 2);
plot(real(constellation_porteuse_12), imag(constellation_porteuse_12), 'x');
title('Constellation ML de la 12ème porteuse');
xlabel('Partie réelle');
ylabel('Partie imaginaire');
axis equal;
grid on;
